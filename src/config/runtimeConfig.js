...
function getBackendUrl() {
  ...
  // Fallback 1: Environment variable (build-time)
  if (process.env.REACT_APP_BACKEND_URL) {
    return process.env.REACT_APP_BACKEND_URL;
  }

  // Fallback 2: Development - still use production backend, but prevent redirects
  // Frontend local connects to Render backend, but stays on localhost after login
  // No local backend needed - use production backend URL

  // Fallback 3: Production default — trebuie aliniat la `public/runtime-config.json` (SSOT deploy edu: backend-server-eu).
  // Nu folosi backend-server-f82y aici: alt serviciu Render / rute inexistente la teste manuale.
  return 'https://backend-server-eu.onrender.com';
}
...
function getAuthBackendUrl() {
  ...
  const env = process.env.REACT_APP_AUTH_BACKEND_URL;
...
function getApiBaseUrl() {
  ...
  // Fallback 2: Derive from backend URL
...
/**
 * Secret pentru header X-Ota-Long-Ops-Secret: runtime-config.json (OTA_LONG_OPS_SECRET) apoi REACT_APP_* la build.
...
function getConfigSource() {
  ...
  if (process.env.REACT_APP_BACKEND_URL || process.env.REACT_APP_API_BASE_URL) {
    return 'REACT_APP_* env vars';
  }
  
  // Check if production default
