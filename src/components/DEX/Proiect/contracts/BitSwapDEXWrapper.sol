...
contract BitSwapDEXWrapper is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public burnPercentage = 50; // 50% din fee-uri sunt burned
    
    /// @notice Fee distribution: percentage pentru stakers (0-100%)
    uint256 public stakersPercentage = 30; // 30% pentru stakers
    
    /// @notice Fee distribution: percentage pentru treasury (0-100%)
    uint256 public treasuryPercentage = 20; // 20% pentru treasury
    ...
    function _distributeFee(address token, uint256 feeAmount) internal {
        ...
        // 2. Burn tokens (trimitem la dead address)
        if (burnAmount > 0) {
            address deadAddress = 0x000000000000000000000000000000000000dEaD;
            IERC20(token).safeTransfer(deadAddress, burnAmount);
        }
        
        // 3. Transfer la stakers contract (placeholder - va fi implementat când avem staking contract)
        // Pentru acum, trimitem la treasury
        if (stakersAmount > 0) {
