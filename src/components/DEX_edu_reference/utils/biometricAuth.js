...
export async function registerBiometric(userId, username) {
  try {
    ...
    localStorage.setItem('bits_biometric_credential_id', credential.id);
...
export async function authenticateBiometric(userId = null) {
  try {
    ...
    const storedCredentialId = localStorage.getItem('bits_biometric_credential_id');
    ...
    // Save user session
    if (result.user) {
      localStorage.setItem('bits_user', JSON.stringify(result.user));
      
      // Save refresh token if provided
      if (result.refreshToken) {
        localStorage.setItem('bits_refresh_token', result.refreshToken);
      }
    }
...
export function hasBiometricCredential() {
  return !!localStorage.getItem('bits_biometric_credential_id');
}
...
export function removeBiometricCredential() {
  localStorage.removeItem('bits_biometric_credential_id');
}
