...
import aiTradingApiService from '../../services/DEX/aiTradingApiService';
...
export function useAITrading(userId) {
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      setStatus(null);
      console.error('[AI Trading] Error loading bot status:', err);
    } finally {
      setRefreshing(false);
    }
  }, [userId]);
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      setError(errorMessage);
      setStats(null);
      console.error('[AI Trading] Error loading bot stats:', err);
    } finally {
      setRefreshing(false);
    }
  }, [userId]);
  ...
  const analyzeMarket = useCallback(async (token, marketData = null) => {
    ...
    try {
      setLoading(true);
      setError(null);
      const response = await aiTradingApiService.analyzeMarket(userId, token, marketData);
      return response;
    } catch (err) {
      const errorMessage = await handleApiError(err);
      setError(errorMessage);
