...
import '../../styles/components/strategy-card.css';
...
const StrategyCard = ({ 
  ...
  const handleDelete = async (e) => {
    ...
    if (window.confirm(`Are you sure you want to delete strategy "${strategy.name}"?`)) {
      try {
        ...
        errorWithPrefix('StrategyCard', 'Error deleting strategy:', error);
      }
    }
  };
  ...
  const handleEnable = async (e) => {
    ...
    try {
      ...
      errorWithPrefix('StrategyCard', 'Error enabling strategy:', error);
    }
  };
  ...
  const handleDisable = async (e) => {
    ...
    try {
      ...
      errorWithPrefix('StrategyCard', 'Error disabling strategy:', error);
    }
  };
  ...
  return (
    <div 
      ...
      role="button"
      tabIndex={0}
      aria-label={`Strategy ${strategy.name}, ${isEnabled ? 'enabled' : 'disabled'}`}
    >
      ...
      <div className="strategy-card-actions">
        ...
        {isEnabled ? (
          <button
            ...
            aria-label={`Disable strategy ${strategy.name}`}
