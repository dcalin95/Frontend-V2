/**
 * StxSwapPage – Pagină Swap Stacks / Alex Protocol.
 */
...
export default function StxSwapPage() {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: 32, paddingBottom: 40 }}>
      ...
      <div style={{ width: '100%', maxWidth: 440, marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 18, fontWeight: 700, color: '#f7931a' }}>₿</span>
          <span style={{ fontWeight: 700, fontSize: 18 }}>STX Swap</span>
          <span style={{ fontSize: 10, fontWeight: 700, background: 'rgba(247,147,26,0.12)', color: '#f7931a', padding: '2px 8px', borderRadius: 10, border: '1px solid rgba(247,147,26,0.3)' }}>Preview</span>
        </div>
        <nav style={{ display: 'flex', gap: 8, fontSize: 13 }}>
          <Link to="/dex-edu/stx/trade" style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 8, border: '1px solid var(--ds-border-color,#27272a)', color: 'var(--ds-text-secondary)', textDecoration: 'none' }}>
      ...
      {/* Wallet banner */}
      {!isConnected && (
        <div style={{ width: '100%', maxWidth: 440, marginBottom: 12, padding: '10px 14px', borderRadius: 10, background: 'rgba(247,147,26,0.06)', border: '1px solid rgba(247,147,26,0.2)', fontSize: 13, color: 'var(--ds-text-secondary)', textAlign: 'center' }}>
          Connect Hiro / Leather wallet to access STX swaps
        </div>
      )}
      ...
      <div style={{ marginTop: 16, fontSize: 11, color: 'var(--ds-text-secondary)', opacity: 0.5, textAlign: 'center' }}>
        Swap STX · sBTC · USDA via Alex Protocol · Stacks mainnet
      </div>
    </div>
  );
}
