 ...
 *
 * Note: Automated OTA AI execution is backend-driven. This module's executeSwap() throws (not implemented).
 * Use Swap panel (swapExecutionService) or backend OTA auto (AITradingExecutor) for real execution.
 * See: docs/OTA_AI_AUTO_WORKER_SPEC.md, scripts/ota-auto-execution-worker-reference.js
 *
...
export class AITradingExecution {
  ...
  async executeSwap({ provider, tokenIn, tokenOut, amountIn, amountOutMin, deadline }) {
    // Production: frontend executeSwap is not implemented. Use Swap panel (swapExecutionService)
    // or backend OTA auto execution (AITradingExecutor). Real execution/API only.
    throw new Error(
      'Frontend executeSwap not implemented. Use Swap panel for manual swaps or backend OTA auto execution.'
    );
  }
